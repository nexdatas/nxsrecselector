...
def check(checkeritem, tangoSourceErrorStates, tangoSourceWarningStates):
    ...
    for ds in checkeritem:
        try:
            ...
            state = dp.state()
            if str(state) in tangoSourceErrorStates:
                raise FaultStateError("%s STATE" % state)
            # if str(state) in tangoSourceOffStates:
            ...
            if not ds.attr:
                ...
                if v[0].has_failed or v[0].value is None:
                    raise Exception("Empty Attribute")
            if str(state) in tangoSourceWarningStates:
