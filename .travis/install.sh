...
if [ $2 = "2" ]; then
    ...
    docker exec -it --user root ndts /bin/sh -c 'export DEBIAN_FRONTEND=noninteractive; apt-get -qq update; apt-get -qq install -y  python-setuptools python-enum34 python-pytango python-tz; apt-get -qq install -y nxsconfigserver-db; sleep 10'
else
