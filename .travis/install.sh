...
docker exec -it --user root ndts /bin/sh -c 'export DEBIAN_FRONTEND=noninteractive;  apt-get -qq install -y  nxsconfigserver-db; sleep 10; apt-get -qq install -y  python-sardana python-nxsconfigserver python-nxswriter python-nxstools'
if [ $? -ne "0" ]
then
    exit -1
fi
