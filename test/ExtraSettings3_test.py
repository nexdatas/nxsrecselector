...
try:
    import Settings3_test
except Exception:
    from . import Settings3_test
...
# test fixture
class ExtraSettings3Test(Settings3_test.Settings3Test):

    # constructor
    # \param methodName name of the test method

    def __init__(self, methodName):
        Settings3_test.Settings3Test.__init__(self, methodName)
...
#                    print "MGS", mg1, mg2, mg3, mg4
                    ...
                    #     self.assertTrue(not lrs.isMntGrpUpdated())
                    #     self.assertTrue(not lrs.isMntGrpUpdated())
                    # except Settings3Test.NotEqualException as e:
...
#                    self.myAssertDict(tmpcf2, ltmpcf)
    ...
    def test_componentClientSources_components(self):
        ...
        for i in range(100):
            ...
            ccp = self._rnd.sample(list(self.mycps.keys()), nccp)
            cps = {}
            for cp in ccp:
                cps[cp] = bool(self._rnd.randint(0, 1))

            nacp = self._rnd.randint(1, len(list(self.mycps.keys())) - 1)
            acp = self._rnd.sample(list(self.mycps.keys()), nacp)
            acps = {}
            for cp in acp:
                acps[cp] = bool(self._rnd.randint(0, 1))
    ...
    def test_componentClientSources_components_var(self):
        ...
        for i in range(100):
            ...
            mem = self._rnd.sample(list(self.mycpsvar.keys()), nmem)
            self._cf.dp.SetCommandVariable(["MCPLIST", json.dumps(mem)])

            nccp = self._rnd.randint(1, len(list(self.mycpsvar.keys())) - 1)
            ccp = self._rnd.sample(list(self.mycpsvar.keys()), nccp)
            cps = {}
            for cp in ccp:
                cps[cp] = bool(self._rnd.randint(0, 1))
            ...
            acp = self._rnd.sample(list(self.mycpsvar.keys()), nacp)
            acps = {}
            for cp in acp:
                acps[cp] = bool(self._rnd.randint(0, 1))
