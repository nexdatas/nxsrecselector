...
## test fixture
class Converter3to2Test(unittest.TestCase):
    ...
    def test_convert_names_pnames(self):
        ...
        for i in range(200):
            ...
            mydsp = dict((ds, self.__rnd.sample([True, False, None], 1)[0])
                         for ds in dss)
            ...
            keys2 = set([self.getRandomName(20) for _ in range(5)])
            for k in keys2:
                if k in names.keys() or \
                   k in names.values():
                    keys2.remove(k)
            keys2.update(keys1)
            for k in keys2:
                if k == 'DataSourcePreselection':
                    mysel[k] = json.dumps(mydsp)
                else:
                    mysel[k] = self.getRandomName(20)
            ...
            for k, vl in osel.items():
                if k == 'DataSourcePreselection':
                    res[names[k]] = json.dumps(ldsp)
                elif k in names:
                    res[names[k]] = vl
                else:
                    res[k] = vl
            if 'DataSourcePreselection' in mysel:
                self.assertEqual(set(mysel['DataSourcePreselection']),
                                 set(json.dumps(ldsp)))
            self.maxDiff = None
            if "MntGrpConfiguration" in res:
                res.pop("MntGrpConfiguration")
